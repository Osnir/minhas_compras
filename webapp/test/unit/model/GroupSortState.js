...
sap.ui.define([
	...
	QUnit.test("Should always return a sorter when sorting", function (assert) {
		...
		assert.strictEqual(this.oGroupSortState.sort("SupplierName").length, 1, "The sorting by SupplierName returned a sorter");
	});
	...
	QUnit.test("Should set the grouping to None if the user sorts by SupplierName and there was a grouping before", function (assert) {
		// Arrange
		this.oModel.setProperty("/groupBy", "GrossAmount");

		this.oGroupSortState.sort("SupplierName");
