			...
			actions : {
						...
						success : function (aEntityData) {
							this.waitFor({
								...
								success : function (oList) {
									var sCurrentId,
										aItemsNotInTheList = aEntityData.filter(function (oObject) {
											return !oList.getItems().some(function (oListItem) {
												return oListItem.getBindingContext().getProperty("PoKey") === oObject.PoKey;
											});
										});

									if (!aItemsNotInTheList.length) {
										// Not enough items all of them are displayed so we take the last one
										sCurrentId = aEntityData[aEntityData.length - 1].PoKey;
									} else {
										sCurrentId = aItemsNotInTheList[0].PoKey;
									}
									...
									oCurrentItem.bindingPath = "/" + oList.getModel().createKey("Headers", {
										PoKey : sCurrentId
									});
				...
				iRememberTheListItem : function (oListItem) {
					...
					this.getContext().currentItem = {
						...
						id: oBindingContext.getProperty("PoKey"),
						title: oBindingContext.getProperty("SupplierName")
					};
				}
			},
			...
			assertions : {
				...
				theListHeaderDisplaysZeroHits : function () {
					return this.waitFor({
						viewName : sViewName,
						id : "page",
						matchers : new PropertyStrictEquals({name : "title", value : "<Headers> (0)"}),
						success : function () {
				...
				theListShouldBeSortedAscendingOnUnitNumber : function () {
					return this.theListShouldBeSortedAscendingOnField("GrossAmount");
				},

				theListShouldBeSortedAscendingOnName : function () {
					return this.theListShouldBeSortedAscendingOnField("SupplierName");
				},
				...
				theListShouldHaveAllEntries : function () {
					...
					// retrieve all Headers to be able to check for the total amount
					...
					return this.waitFor({
						...
						success : function (oList) {
							...
							this.waitFor({
								id : "page",
								viewName : sViewName,
								matchers : new PropertyStrictEquals({name : "title", value : "<Headers> (" + iExpectedLength + ")"}),
								success : function () {
