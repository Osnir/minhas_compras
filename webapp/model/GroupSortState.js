sap.ui.define([
	...
	return BaseObject.extend("ovly.mm.minhas_compras.model.GroupSortState", {
		 ...
		 * @alias ovly.mm.minhas_compras.model.GroupSortState
		 */
		...
		/**
		 * Sorts by SupplierName, or by GrossAmount
		 *
		...
		/**
		 * Groups by GrossAmount, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "GrossAmount") {
				...
				this._oViewModel.setProperty("/sortBy", "GrossAmount");

				aSorters.push(
					new Sorter("GrossAmount", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "SupplierName");
			}
